...
import { PageHeader, FastPlanEntryCard, FastPlanEntry } from '../components';
import { 
    fetchFastPlanEntries, 
    deleteFastPlanEntry, 
    FastPlanEntry as ApiFastPlanEntry 
} from '../api/fast';

export default function FastPlanEntriesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const [entries, setEntries] = useState<FastPlanEntry[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    // Transform API entry to component format
    const transformApiEntry = (apiEntry: ApiFastPlanEntry): FastPlanEntry => {
        ...
        return {
            ...
            fair: apiEntry.fair,
            apologies: apiEntry.apologies,
            stickToValues: apiEntry.stickToValues,
            truthful: apiEntry.truthful,
    ...
    const loadEntries = async () => {
        try {
            setIsLoading(true);
            setError(null);
            const apiEntries = await fetchFastPlanEntries();
            ...
            console.error('Failed to load fast plan entries:', err);
    ...
    const handleView = (entryId: string) => {
        // TODO: Navigate to Fast Plan Entry Detail screen when created
