...
import fastExercisesData from '../data/fastExercises.json';

export default function FastExercisesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const { title, introText, introTitle, exercises } = fastExercisesData;

    const handleExercisePress = (exercise: any) => {
        if (exercise.title === 'FAST Plan') {
            dissolveTo('Learn_FastPlan');
        } else if (exercise.title === 'Start Small') {
            dissolveTo('Learn_FastStartSmall');
        } else {
            ...
            >
                ...
                {exercises.map((exercise) => {
                    if (exercise.hasHeartIcon) {
                        // Custom card for "Build Self-Respect" with heart icon
                        return (
                            <View
                                key={exercise.id}
