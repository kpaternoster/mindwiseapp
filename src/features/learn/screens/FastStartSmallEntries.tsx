...
import { 
    fetchStartSmallEntries, 
    deleteStartSmallEntry, 
    StartSmallEntry as ApiStartSmallEntry 
} from '../api/fast';

export default function FastStartSmallEntriesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const [entries, setEntries] = useState<FastStartSmallEntry[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    // Transform API entry to component format
    const transformApiEntry = (apiEntry: ApiStartSmallEntry): FastStartSmallEntry => {
        ...
        return {
            ...
            date: dateString,
            time: timeString,
            smallRequest: apiEntry.smallRequest,
            outcome: apiEntry.smallOutcome,
            reflection: apiEntry.smallReflection,
            biggerSituation: apiEntry.biggerSituation,
            practiceOutLoud: apiEntry.practice,
            biggerChallengeOutcome: apiEntry.biggerReflection,
            learnedAboutFast: apiEntry.overallReflection,
            nextSteps: apiEntry.nextSteps,
        };
    };
    ...
    const loadEntries = async () => {
        try {
            setIsLoading(true);
            setError(null);
            const apiEntries = await fetchStartSmallEntries();
            ...
            console.error('Failed to load start small entries:', err);
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            // Delete from API
            await deleteStartSmallEntry(id);
        } catch (err) {
