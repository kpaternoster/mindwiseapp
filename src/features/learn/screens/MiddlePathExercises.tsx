...
import middlePathExercisesData from '../data/middlePathExercises.json';

export default function MiddlePathExercisesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const { title, intro, exercises, conclusion } = middlePathExercisesData;

    const handleExercisePress = (exerciseTitle: string) => {
        switch (exerciseTitle) {
            case 'Recognizing Dialectical Dilemmas':
                dissolveTo('Learn_RecognizingDialecticalDilemmas');
                break;
            case 'Acceptance & Change':
