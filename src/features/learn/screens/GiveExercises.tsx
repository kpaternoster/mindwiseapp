...
import giveExercisesData from '../data/giveExercises.json';

export default function GiveExercisesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const { title, introText, introTitle, exercises } = giveExercisesData;

    const handleExercisePress = (exercise: any) => {
        if (exercise.title === 'GIVE Reframing') {
            dissolveTo('Learn_GiveReframing');
        } else if (exercise.title === 'GIVE Practice') {
            dissolveTo('Learn_GivePractice');
        } else {
            ...
            >
                ...
                {exercises.map((exercise) => {
                    if (exercise.hasHeartIcon) {
                        // Custom card for "Build Connection" with heart icon
                        return (
                            <View
                                key={exercise.id}
