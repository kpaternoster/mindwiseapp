...
import { 
    fetchIdentifyEmotionalUrgesEntries, 
    deleteIdentifyEmotionalUrgesEntry, 
    IdentifyEmotionalUrgesEntry as ApiIdentifyEmotionalUrgesEntry 
} from '../api/differentAction';

export default function DifferentActionIdentifyEmotionalUrgesEntriesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const [entries, setEntries] = useState<DifferentActionIdentifyEmotionalUrgesEntry[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    // Group API entries by timestamp (same date/time) and combine into component format
    const groupAndTransformEntries = (apiEntries: ApiIdentifyEmotionalUrgesEntry[]): DifferentActionIdentifyEmotionalUrgesEntry[] => {
        // Group entries by timestamp (rounded to the nearest minute to account for slight timing differences)
        const grouped = new Map<string, ApiIdentifyEmotionalUrgesEntry[]>();
        ...
        const transformedEntries: DifferentActionIdentifyEmotionalUrgesEntry[] = [];
        ...
        grouped.forEach((group, key) => {
            ...
            // Combine all urge pairs from this group
            const urgePairs = group.map(entry => ({
                urge: entry.urges,
                oppositeAction: entry.oppositeAction,
            }));
    ...
    const loadEntries = async () => {
        try {
            ...
            console.error('Failed to load identify emotional urges entries:', err);
    ...
    const handleDelete = async (entryId: string) => {
        try {
            // Find the entry to get all its urge pairs
            ...
            await Promise.all(
                entriesToDelete.map(apiEntry => deleteIdentifyEmotionalUrgesEntry(apiEntry.id))
            );
