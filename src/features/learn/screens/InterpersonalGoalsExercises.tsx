...
import interpersonalGoalsExercisesData from '../data/interpersonalGoalsExercises.json';

export default function InterpersonalGoalsExercisesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const { title, introText, introTitle, exercises } = interpersonalGoalsExercisesData;

    const handleExercisePress = (exercise: any) => {
        if (exercise.title === 'Clarify Your Goals') {
            dissolveTo('Learn_ClarifyYourGoals');
        } else if (exercise.title === 'Plan Your Response') {
            dissolveTo('Learn_PlanYourResponse');
        } else {
            ...
            >
                ...
                {exercises.map((exercise) => {
                    if (exercise.hasHeartIcon) {
                        // Custom card for "Balance Your Goals" with heart icon
                        return (
                            <View
                                key={exercise.id}
