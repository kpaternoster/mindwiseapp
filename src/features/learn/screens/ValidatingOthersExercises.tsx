...
import validatingOthersExercisesData from '../data/validatingOthersExercises.json';

export default function ValidatingOthersExercisesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const { title, introText, introTitle, exercises } = validatingOthersExercisesData;

    const handleExercisePress = (exercise: any) => {
        if (exercise.title === 'Self-Reflection') {
            dissolveTo('Learn_SelfReflection');
        } else if (exercise.title === 'Validation Practice') {
            dissolveTo('Learn_ValidationPractice');
        } else {
            ...
            >
                ...
                {exercises.map((exercise) => {
                    if (exercise.hasHeartIcon) {
                        // Custom card for "Connection Through Understanding" with heart icon
                        return (
                            <View
                                key={exercise.id}
