...
import thinkExercisesData from '../data/thinkExercises.json';

export default function ThinkExercisesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const { title, introText, introTitle, exercises } = thinkExercisesData;

    const handleExercisePress = (exercise: any) => {
        if (exercise.title === 'Communicate with THINK') {
            dissolveTo('Learn_CommunicateWithThink');
        } else {
            ...
            >
                ...
                {exercises.map((exercise) => {
                    if (exercise.hasHeartIcon) {
                        // Custom card for "Practice Thoughtful Communication" with heart icon
                        return (
                            <View
                                key={exercise.id}
