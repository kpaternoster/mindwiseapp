...
import hereAndNowExercisesData from '../data/hereAndNowExercises.json';

export default function HereAndNowExercisesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const { title, introText, introTitle, exercises } = hereAndNowExercisesData;

    const handleExercisePress = (exercise: any) => {
        if (exercise.title === 'Mindful Presence Practice') {
            dissolveTo('Learn_MindfulPresencePractice');
        } else {
            // Finding Presence doesn't have navigation
            ...
            >
                ...
                {exercises.map((exercise) => {
                    if (exercise.hasHeartIcon) {
                        // Custom card for "Finding Presence" with heart icon
                        return (
                            <View
                                key={exercise.id}
