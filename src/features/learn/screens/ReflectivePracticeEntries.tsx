...
import { fetchReflectivePracticeEntries, deleteReflectivePracticeEntry } from '../api/stop';

export default function ReflectivePracticeEntriesScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const [entries, setEntries] = useState<ReflectivePracticeEntry[]>([]);
    ...
    const loadEntries = async () => {
        ...
        try {
            const apiEntries = await fetchReflectivePracticeEntries();

            // Transform API entries to component format
            const transformedEntries: ReflectivePracticeEntry[] = apiEntries.map((apiEntry) => {
            ...
            console.error('Failed to load reflective practice entries:', err);
    ...
    const handleDelete = async (entryId: string) => {
        ...
        try {
            await deleteReflectivePracticeEntry(entryId);
        } catch (err) {
