...
import halfSmilingMindfulHalfSmilePracticeData from '../data/halfSmilingMindfulHalfSmilePractice.json';
import { createMindfulHalfSmilePracticeEntry } from '../api/halfSmiling';
...
export default function HalfSmilingMindfulHalfSmilePracticeScreen() {
    const { dissolveTo } = useDissolveNavigation();
    const { title, howToPractice, timerDuration, timerTitle, inputs, buttons } = halfSmilingMindfulHalfSmilePracticeData;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const entryData = {
                calmingVisualization: inputValues.visualization.trim(),
                bodyResponse: inputValues.bodyResponse.trim(),
                reflection: inputValues.reflection.trim(),
            };

            // Save to API
            await createMindfulHalfSmilePracticeEntry(entryData);
            ...
            console.error('Failed to save mindful half smile practice entry:', err);
    ...
    const handleView = () => {
        dissolveTo('Learn_HalfSmilingEntries', { initialTab: 'practice' });
    };
                        ...
                        <View style={{ width: TIMER_SIZE, height: TIMER_SIZE, position: 'relative', alignItems: 'center', justifyContent: 'center' }}>
                            ...
                            <View className="items-center" style={{ zIndex: 1 }}>
                                ...
                                <Text style={[t.textRegular, { color: colors.text_secondary }]} className="mt-2">
                                    {isRunning ? 'Smile Gently..' : isComplete ? 'Complete' : 'Ready'}
                                </Text>
                            </View>
                        </View>
