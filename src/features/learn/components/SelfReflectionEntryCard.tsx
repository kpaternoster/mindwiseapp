...
export interface SelfReflectionEntry {
    ...
    whatEmotions?: string;
    whatResponse?: string;
    howValidateOthers?: string;
}

interface SelfReflectionEntryCardProps {
    entry: SelfReflectionEntry;
    onView: (entryId: string) => void;
    onDelete: (entryId: string) => void;
}

export const SelfReflectionEntryCard: React.FC<SelfReflectionEntryCardProps> = ({
    entry,
    onView,
    onDelete,
}) => {
    ...
    // Get snippet text - prioritize describeTime
    const getSnippet = () => {
        if (entry.describeTime) {
            return `Situation: ${truncateText(entry.describeTime, 60)}`;
        }
        if (entry.whatEmotions) {
            return `Emotions: ${truncateText(entry.whatEmotions, 60)}`;
        }
